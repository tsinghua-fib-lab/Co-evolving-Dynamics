...
class NetworkDynamics:
    ...
    def reset(self, gamma_plus, gamma_minus, r_plus, r_minus):
        for k,v in gamma_plus.items():
            if v is not None:
                if k == 'AB':
                    self.gamma_plus_AB = v
...
def bipartite_simulation(
    ...
    gp_new_keywords = [k+'{:.2f}'.format(v*1000) for k,v in gamma_plus_new.items() if v is not None]
    gm_keywords = [k+'{:.2f}'.format(v*1000) for k,v in gamma_minus.items() if v is not None]
    gm_new_keywords = [k+'{:.2f}'.format(v*1000) for k,v in gamma_minus_new.items() if v is not None]
    ...
    gp_new_keywords = '-'.join(gp_new_keywords)
    gm_keywords = '-'.join(gm_keywords)
    gm_new_keywords = '-'.join(gm_new_keywords)
    a_keywords = '-'.join(a_keywords)
    rp_keywords = '-'.join(rp_keywords)
    rm_keywords = '-'.join(rm_keywords)

    if not isinstance(sigma, list):
        output_path = './results_rp{}_rm{}/alpha{},gp{},gpnew{},gm{},gmnew{},s{:.2f}/'.format(rp_keywords, rm_keywords, a_keywords, gp_keywords, gp_new_keywords, gm_keywords, gm_new_keywords, sigma*1000)
        if type(extended) == str:
            output_path = './results_rp{}_rm{}/alpha{},gp{},gpnew{},gm{},gmnew{},s{:.2f}{extended}/'.format(rp_keywords, rm_keywords, a_keywords, gp_keywords, gp_new_keywords, gm_keywords, gm_new_keywords, sigma*1000, extended = extended)
        print(output_path)
    elif len(sigma) == 2:
        output_path = './results_rp{}_rm{}/alpha{},gp{},gpnew{},gm{},gmnew{},s{:.2f}/'.format(rp_keywords, rm_keywords, a_keywords, gp_keywords, gp_new_keywords, gm_keywords, gm_new_keywords, max(sigma)*1000)
        if type(extended) == str:
            output_path = './results_rp{}_rm{}/alpha{},gp{},gpnew{},gm{},gmnew{},s{:.2f}{extended}/'.format(rp_keywords, rm_keywords, a_keywords, gp_keywords, gp_new_keywords, gm_keywords, gm_new_keywords, max(sigma)*1000, extended = extended)
        print(output_path)
    ...
    edge_list = ND.edge_evolving(edge_list=bidataset.edge_index, states=states, init = True) # 0 开始
    ...
    for t in tqdm.tqdm(range(num_runs)):
        ...
        if t == change_round:
            ND.reset(gamma_plus = gamma_plus_new, gamma_minus = gamma_minus_new, r_plus = r_plus_new, r_minus = r_minus_new)
            print('Reset new parameters!')
